...
/*
notes from daryl for encryption
String - byte array - bouncycastle shit - hex (using utils) ->transmit
transmit -> hex - byte array - bouncy castle decrypt - byte array (to string)
 */
...
public class Server {
    ...
    public void run() throws IOException {
        ...
        while (true) {
            ...
            //SEND PUBLIC KEY FROM SERVER TO USER newUser.getOutSTream.println(PUBLICKEY+ pk);
            // add newUser message to list
            this.clients.add(newUser);
byte[] key = newUser.encryption.getPubKey();
            newUser.getOutStream().println("{{{" + Utils.toHex(key));
            System.out.println("hello");
            // Welcome msg
            newUser.getOutStream().println(newUser.encryption.encrypt(Utils.toByteArray(
    ...
    public void broadcastMessages(String msg, User userSender) {
        for (User client : this.clients) {
            msg = Utils.toString(client.encryption.encrypt(Utils.toByteArray(msg)));
    ...
    public void sendMessageToUser(String msg, User userSender, String user) {
        boolean find = false;
        msg = Utils.toString(userSender.encryption.encrypt(Utils.toByteArray(msg)));
        for (User client : this.clients) {
    ...
    public void run() {
        ...
        while (sc.hasNextLine()) {
            ...
            if (message.startsWith("{{{")) {
                ...
                user.encryption.setKB(keyBytes);
            } else {
                try {
                    message = Utils.toString(user.encryption.decrypt(Utils.toByteArray(message)));
                    //
